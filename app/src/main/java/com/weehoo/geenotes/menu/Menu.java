...
public class Menu {
    ...
    public void addItem(MenuItem menuItem, MenuAlign menuAlign) {
        switch (menuAlign) {
            case MENU_ALIGN_LEFT: {
                mLeftItems.add(menuItem);
            } break;

            case MENU_ALIGN_RIGHT: {
                mRightItems.add(menuItem);
            } break;
        }
     ...
     * @param point The point from which the Menu rectangle will originate.
     ...
     */
    public void draw(Canvas canvas, PointF point, MenuAnchor menuAnchor, Paint paint) {
        mRect = null;

        // Determine menu rect.
        switch (menuAnchor) {
            case MENU_ANCHOR_TOP_LEFT: {
                mRect = new RectF(point.x, point.y, point.x + MENU_ITEM_WIDTH, point.y + MENU_ITEM_HEIGHT);
            } break;

            case MENU_ANCHOR_BOTTOM_RIGHT: {
                mRect = new RectF(point.x - MENU_ITEM_WIDTH, point.y - MENU_ITEM_HEIGHT, point.x, point.y);
            } break;
        }
        ...
        for (int i = 0; i < mLeftItems.size(); i++) {
            ...
            // Draw left item.
            (new Thread(new MenuItemDrawRunnable(canvas, itemRect, mLeftItems.get(i).getmBitmap(), paint))).start();
        }
        ...
        for (int i = 0; i < mRightItems.size(); i++) {
            ...
            // Draw right item.
            (new Thread(new MenuItemDrawRunnable(canvas, itemRect, mRightItems.get(i).getmBitmap(), paint))).start();
        }
    ...
    private class MenuItemDrawRunnable implements Runnable
    {
         ...
         * @param bitmap Menu item icon.
         * @param paint Drawing paint.
         */
        public MenuItemDrawRunnable(Canvas canvas, RectF rect, Bitmap bitmap, Paint paint) {
            mCanvas = canvas;
            mBitmap = bitmap;
            mRect = rect;
            mPaint = paint;
